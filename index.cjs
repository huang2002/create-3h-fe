...
program
    ...
    .then(async (args) => {
        ...
        await fsPromises.writeFile(
            '.gitignore',
            ['node_modules', 'types', 'js', 'dist'].join('\n') + '\n',
        );
        await fsPromises.writeFile(
            'package.json',
            JSON.stringify(
                {
                    ...
                    type: 'commonjs',
                    module: `./dist/${name}.min.mjs`,
                    main: `./dist/${name}.umd.min.cjs`,
                    types: './index.d.ts',
                    exports: {
                        types: './index.d.ts',
                        node: {
                            import: `./dist/${name}.mjs`,
                            default: `./dist/${name}.umd.cjs`,
                        },
                        default: {
                            import: `./dist/${name}.min.mjs`,
                            default: `./dist/${name}.umd.min.cjs`,
                        },
                    },
                    author: data.author,
                    license: 'MIT',
                    ...
                    repository:
                        args.getOption('--repo')[0] || `${data.author}/${name}`,
